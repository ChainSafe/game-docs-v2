...
Yes, you can make Desktop / Web and Mobile games using our SDK. You will need to create a Web3GL build to make desktop / mobile games with web3.unity.

### I want to connect to other EVM-compatible chains but it only lets me connect to Ethereum. How do I do that?

You will need to edit the `network.js` file found within the **WebTemplates** folder. Edit the `ChainId` to the one for the blockchain you want your game to interact with. See websites like [Chainlist](https://chainlist.org/) for a list of EVM ChainID's.
...
## Common Questions About web3.unity v2

### How do I download v2?
...
### What are the benefits of moving to v2?
...
### Are code changes required when I update to v2?
The update will require code changes for games using web3.unity v1.6.x and below (legacy).
...
### How long will it take to migrate my game from v1.6 (or lower) to v2?
...
### My game is currently running v1.6 (or lower). What are the steps I need to take to switch over to v2?
...
Without these changes, people will be unable to make transactions from within existing games. This will require some effort to refactor your code. Users can expect payoffs in the longer term, and we’re here to help make the transition as smooth as possible.
...
The new decentralized version of the SDK will significantly reduce the involvement our intermediary API servers play which previously was causing developers with intermittent outages and bottlenecks. Instead, we are encouraging users to assess and determine the uptime guarantees needed from infrastructure providers in supplying an RPC node.

### Will I still be able to use v1.6.x (or lower) with my game?
    
The aim is to move away from supporting the API service once the community has had adequate time to upgrade to v2.x.
    
The sunset date for the API is slated for May, 2023.
    
### My current build is running just fine, is upgrading compulsory?
    
While we appreciate and understand how this may impact some of our existing developers and their games/communities, an upgrade is required. 
    
We believe the net positive as a result of ultimate developer control, transparency, and decentralization makes this upgrade a worthwhile investment in the long run.
    
### Can someone help migrate my game to v2?
